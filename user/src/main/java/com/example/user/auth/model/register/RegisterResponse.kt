...
internal fun RegisterResponse.toTokens(): Tokens? {
    val accessToken =
        this.included.find { it.type == TYPE_SESSION }?.attributes?.get(ATTRIBUTE_ACCESS_TOKEN)
    val refreshToken =
        this.included.find { it.type == TYPE_SESSION }?.attributes?.get(ATTRIBUTE_REFRESH_TOKEN)
